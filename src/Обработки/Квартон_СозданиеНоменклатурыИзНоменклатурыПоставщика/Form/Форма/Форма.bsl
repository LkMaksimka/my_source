...
Процедура ВидНоменклатурыПриИзменении(Элемент)
	НоменклатурнаяГруппа = ПустаяНоменклатурнаяГруппа();
КонецПроцедуры

&НаКлиенте
Процедура НоменклатурнаяГруппаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОткрытьНастроеннуюФормуВыбора(ВидНоменклатуры, ЭтаФорма);
КонецПроцедуры
...
&НаКлиенте
Процедура ТаблицаВидНоменклатурыПриИзменении(Элемент)
	ТекСтрока = Элементы.Таблица.ТекущиеДанные;
	ТекСтрока.НоменклатурнаяГруппа = ПустаяНоменклатурнаяГруппа();
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаНоменклатурнаяГруппаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ТекСтрока = Элементы.Таблица.ТекущиеДанные;
	ОткрытьНастроеннуюФормуВыбора(ТекСтрока.ВидНоменклатуры, Элементы.Таблица);
КонецПроцедуры
...
Процедура ТаблицаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.Номенклатура") Тогда
		ТекСтрока = Элементы.Таблица.ТекущиеДанные;
		ТекСтрока.НоменклатурнаяГруппа = ВыбранноеЗначение;
	КонецЕсли; 
КонецПроцедуры
...
Процедура СоздатьНоменклатуру(Команда)
	...
	Для каждого ИдСтроки Из ВыделенныеСтроки Цикл
		...
		Если НЕ Строка.НоменклатурнаяГруппа.Пустая() 
			...
			Структура.Вставить("НоменклатурнаяГруппа", Строка.НоменклатурнаяГруппа);
...
Процедура СоздатьНоменклатуруНаСервере(МассивПараметровСоздания, ЕдиницыИзмерения)
	...
	Для каждого ЭлементМасс Из МассивПараметровСоздания Цикл
		...
		СпрНоваяНоменклатура.Родитель = ЭлементМасс.НоменклатурнаяГруппа;
...
Функция кв_ПолучитьГруппуПоПравиламЗаполнения(кв_ВидНоменклатуры)
	кв_Список = РегистрыСведений.кв_ПравилаУказанияГруппыДляНоменклатуры.ПолучитьГруппыНоменклатуры(кв_ВидНоменклатуры);
	Возврат кв_Список; 
КонецФункции

&НаКлиенте
Функция ПустаяНоменклатурнаяГруппа()
	НоменклатурнаяГруппа = ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка");
	Возврат НоменклатурнаяГруппа;
КонецФункции

&НаКлиенте
Процедура ОткрытьНастроеннуюФормуВыбора(ВидНоменклатуры, кв_Владелец)
	...
	ОткрытьФорму("Справочник.Номенклатура.ФормаВыбораГруппы", СтруктураОтбора, кв_Владелец);
