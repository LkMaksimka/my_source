...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Версия = "1.1.0.0";
...
функция ПолучитьДанныеСервераKupitProxy()
	...
	//url = "http://kupit-proxy.ru/api/GetProxiesTxt?key=1025fdd8-585d-4934-a90a-16e3d86e1142&protocols=https&anonymityLevels=elite&uprate=80;   //+"&"+services
	url = "http://kupit-proxy.ru/api/GetProxiesTxt?key="+Proxykey+"&"+protocols+"&"+anonymityLevels+"&"+uprate;
...
Процедура ЗаполнитьСправочник(ВремменыйФайл)
	...
	Пока Стр <> Неопределено Цикл 
		...
		Если ЗаченияПрокси.Количество() > 0 Тогда
			ЭтоIP4 = СтрРазделить(ЗаченияПрокси[0],".").Количество() = 4;
