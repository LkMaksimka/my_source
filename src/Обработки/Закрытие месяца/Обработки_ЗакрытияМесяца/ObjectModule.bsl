...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Версия = "1.0.2.1";
...
// Описание: БайковаЭИ - может продовать товар любой организации,
// товар КомпТехнологии могут продовать только: СОФТКОМПЬЮТЕР, КВАРТОНК, ПАРТНЕРНН. 
Процедура ОбновитьНастройкиПередачиТоваровМеждуОрганизациями(Знач ВидЦены, Знач ЭлементВладелец, Знач ЭлементПродавец)
	...
	Если ЭлементПродавец = СОФТКОМПЬЮТЕР  ИЛИ ЭлементПродавец = КВАРТОНК ИЛИ ЭлементПродавец = ПАРТНЕРНН Тогда
		МожетПродоватьТоварКомпТехнологии = Истина;
	Иначе 
		МожетПродоватьТоварКомпТехнологии = Ложь;
	КонецЕсли; 
	...
	Если ТекЗапись.Выбран() Тогда
		...
		Если БайковаЭИ = ЭлементПродавец  ИЛИ (КомпТехнологии = ЭлементВладелец И МожетПродоватьТоварКомпТехнологии) Тогда
			ТекЗапись.СпособПередачиТоваров = Перечисления.СпособыПередачиТоваров.Продажа;
			ТекЗапись.ВидЦены = ВидЦены;
		Иначе
