...
Процедура ОчиститьВесьРегистрРезервовОрганизации(Период, ПоНеПроведеннымДокументам) Экспорт
	...
	Если Период <> Дата("00010101") Тогда
	...
	Для каждого ЭлементВладелец Из СписокОрганизаций Цикл
		...
		Для каждого ЭлементПродавец Из СписокОрганизаций  Цикл
			...
			Если ТекЗапись.Выбран() Тогда
				...
				ИначеЕсли КомпТехнологии = ЭлементВладелец И СОФТКОМПЬЮТЕР = ЭлементПродавец Тогда
					ТекЗапись.СпособПередачиТоваров = Перечисления.СпособыПередачиТоваров.Продажа;
					ТекЗапись.ВидЦены = ВидЦены;
					ТекЗапись.Записать();
					
				ИначеЕсли   КомпТехнологии = ЭлементВладелец И КВАРТОНК = ЭлементПродавец Тогда
					ТекЗапись.СпособПередачиТоваров = Перечисления.СпособыПередачиТоваров.Продажа;
					ТекЗапись.ВидЦены = ВидЦены;
					ТекЗапись.Записать();
					
				ИначеЕсли   КомпТехнологии = ЭлементВладелец И ПАРТНЕРНН = ЭлементПродавец Тогда	
					ТекЗапись.СпособПередачиТоваров = Перечисления.СпособыПередачиТоваров.Продажа;
					ТекЗапись.ВидЦены = ВидЦены;
					ТекЗапись.Записать();
				Иначе
