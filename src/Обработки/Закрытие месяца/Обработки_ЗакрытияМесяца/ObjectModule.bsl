...
Процедура ОбновитьНастройкиПередачиТоваровМеждуОрганизациями(Знач ВидЦены, Знач ЭлементВладелец, Знач ЭлементПродавец)
	...
	Если СОФТКОМПЬЮТЕР = ЭлементПродавец ИЛИ КВАРТОНК = ЭлементПродавец ИЛИ ПАРТНЕРНН = ЭлементПродавец Тогда
		МожетПродоватьОтКомпТехнологии = Истина;
	Иначе 
		МожетПродоватьОтКомпТехнологии = Ложь;
	КонецЕсли; 
	...
	Если ТекЗапись.Выбран() Тогда
		...
		Если БайковаЭИ = ЭлементПродавец Тогда
			ДоЗаполнитьЗаписьРегистра(ВидЦены, Перечисления.СпособыПередачиТоваров.Продажа, ТекЗапись);
			
		ИначеЕсли КомпТехнологии = ЭлементВладелец И МожетПродоватьОтКомпТехнологии Тогда
			
			ДоЗаполнитьЗаписьРегистра(ВидЦены, Перечисления.СпособыПередачиТоваров.Продажа, ТекЗапись);
			
		Иначе
			ПустойВидЦены = Справочники.ВидыЦен.ПустаяСсылка();
			ДоЗаполнитьЗаписьРегистра(ПустойВидЦены, Перечисления.СпособыПередачиТоваров.НеПередается, ТекЗапись);
		КонецЕсли; 
...
Процедура ДоЗаполнитьЗаписьРегистра(Знач ВидЦены, Знач СпособыПередачиТоваров, ТекЗапись)
	
	ТекЗапись.СпособПередачиТоваров = СпособыПередачиТоваров;
	ТекЗапись.ВидЦены = ВидЦены;
