...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
    Если ИсточникВыбора.ИмяФормы = "Обработка.тн_МассовоеПеремещениеОС.Форма.ФормаЗаполнения" Тогда
        ...
        КонецЦикла;
        НеНайденыеКоды = ВыбранноеЗначение.НеНайденыеКоды;
        Для каждого Элемент Из НеНайденыеКоды Цикл
...
Процедура ПерейтиКСпискуДокументов(Команда)
    ...
    Форма = ПолучитьФорму("Документ.ПеремещениеОС2_4.ФормаСписка", ПараметрыФормы);
    Форма.Открыть();
