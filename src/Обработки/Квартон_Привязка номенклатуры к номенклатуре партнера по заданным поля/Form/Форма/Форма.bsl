...
Процедура Отобрать(Команда)
    НеобходимыеПоляЗаполенены = ПроверитьЗаполнениНеобходимыхПолей();
	Если НЕ НеобходимыеПоляЗаполенены Тогда
...
Процедура ОтобратьСервер()
    ...
    Если РежимПоиска = "Равно" Тогда
       ...
       Иначе
        УсловиеСоединения = "ПОДОБНО ""%"" + &ПолеНоменклатуры + ""%"" ";  
    КонецЕсли; 
    ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеСоединения", УсловиеСоединения);
    
    ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ПолеНоменклатурыПоставщика", "НоменклатураПоставщиков." + ПолеНоменклатурыПоставщика);
...
Функция ПроверитьЗаполнениНеобходимыхПолей()
    ...
    Если Партнер.Пустая() 
        ...
        ИЛИ ПустаяСтрока(ПолеНоменклатурыПоставщика) Тогда
