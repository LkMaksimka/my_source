...
Функция СведенияОВнешнейОбработке() Экспорт
    ...
    ПараметрыРегистрации.Версия = "1.0.0.5";
    ПараметрыРегистрации.БезопасныйРежим = Истина;
    ПараметрыРегистрации.Вставить("Назначение", МассивНазначений);

    Команда = ПараметрыРегистрации.Команды.Добавить();
    Команда.Представление = НСтр("ru = '(ТН) Авансовый отчет'");
...
Функция СформироватьПечатнуюФормуАвансовогоОтчета(МассивОбъектов, ОбъектыПечати)
    ...
    ШаблонОшибки = НСтр("ru = 'Печать АО-1 используется только для документов с операцией 
                        |""Закупка через подотчетное лицо"".';
						|en = 'You should print AO-1 only for documents with the 
                        |""Purchase through advance holder"" operation.'");
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьТитулПодвал.Параметры.тн_ОтветственныйУтвердил = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(Выборка.тн_ОтветственныйУтвердил);
		...
		// 1С Рарус МаксимовМГ - заявка \  31.03.2020 ++
		ДолжностьПодотчетногоЛица = ПолучитьДолжностьФизЛица(Выборка.ПодотчетноеЛицо);
		ОбластьТитул.Параметры.ДолжностьПодотчетногоЛицаПредставление = ДолжностьПодотчетногоЛица;
		Если ПустаяСтрока(ДолжностьПодотчетногоЛица) Тогда
			СообщениеПользователю = Новый СообщениеПользователю;
			СообщениеПользователю.Текст = "Не удалось получить должность. Сформируйте печатную форму еще раз.";
...
Функция ТекстЗапросаПечати()
	...
	|	ДанныеДокумента.ПодотчетноеЛицо.Код	КАК ТабельныйНомер,
	|	ДанныеДокумента.тн_ОтветственныйУтвердил КАК тн_ОтветственныйУтвердил,
...
Функция ПолучитьДолжностьФизЛица(ФизЛицо)
	...
	ПолучаемыеДанные.Добавить("Код");

	КадровыеДанные = КадровыйУчет.КадровыеДанныеФизическихЛиц(Истина, ФизЛицо, ПолучаемыеДанные, ТекущаяДата());
	
	Если КадровыеДанные.Количество() > 0 Тогда
		ТабельныйНомер = КадровыеДанные[0].Код;
	Иначе
		ТабельныйНомер = "00000000";
	КонецЕсли;
	
	
	ТабельныйНомер = Прав(СтрЗаменить(ТабельныйНомер,"-",""),8);
	
	Прокси = WSСсылки.WSСсылка1.СоздатьWSПрокси("http://cob.ru/1C_ERP_BU/get_personal_data/",
												"mi_req_get_personal_dataService",
												"mi_req_get_personal_dataPort");
	Прокси.Пользователь = "XI_1CERP_TN";
	Прокси.Пароль 		= "1qa@WS3ed";
	
	ТипОбъектаСотрудника = Прокси.ФабрикаXDTO.Тип("urn:sap-com:document:sap:rfc:functions", "ZFM_GET_PERSONAL_DATA");
	
	Попытка
		ОбъектСотрудник = Прокси.ФабрикаXDTO.Создать(ТипОбъектаСотрудника);
		ОбъектСотрудник.PERNR = Прокси.ФабрикаXDTO.Создать(ТипОбъектаСотрудника.Свойства.Получить("PERNR").Тип, ТабельныйНомер);
		ОбъектСотрудник.DATUM = Прокси.ФабрикаXDTO.Создать(ТипОбъектаСотрудника.Свойства.Получить("DATUM").Тип, ТекущаяДата());
		
		Ответ = Прокси.mi_req_get_personal_data(ОбъектСотрудник);
	Исключение
		Должность = "";
	КонецПопытки; 	
